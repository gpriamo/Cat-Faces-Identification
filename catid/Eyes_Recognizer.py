...
# def test(model: cv.face_BasicFaceRecognizer):
#     mod, hei = train_recongizer(model, "../dataset_info/best.csv", resize=True)
#     predict(model=mod, height=hei, resize=True, probe_image="../images/dataset/cropped/s13/9.jpg", probe_label=6, identification=True)
...
if __name__ == '__main__':
    ...
    avgs = rc.evaluate_avg_performances(model, test_thresholds, k_fold_files)
    eye_avgs = rc.evaluate_avg_performances(model, test_thresholds, k_fold_files, use_eyes=True)

    plot_error_rates([avgs, eye_avgs], [model_name + ' without eye color detection', model_name + ' with eye color detection'])
    plot_rocs([avgs, eye_avgs], [model_name + ' without eye color detection', model_name + ' with eye color detection'])
